export default function ResultCard({ result }) {
  ...
  return (
    <div className="mt-6 p-6 w-full max-w-lg bg-white rounded-lg shadow-lg">
      ...
      <p className="text-gray-600 mb-2"><strong>Claim:</strong> {result.text || "No claim provided."}</p>
      
      <p className="text-gray-700 mb-2">
        <strong>Best Verdict:</strong> 
        {result.best_ai_verdict !== "No Google Fact Check available"
        ...
        <>
          ...
          <ul className="list-disc list-inside mt-2">
            {result.ai_verdicts.map((item, index) => (
              <li key={index} className="text-gray-600">{item.label}: {item.confidence}%</li>
            ))}
          </ul>
        </>
      ) : (
      ...
      {result.google_fact_checks.length === 0 && result.google_search_results && (
        <>
          ...
          <ul className="list-disc list-inside mt-2">
            {result.google_search_results.map((item, index) => (
              <li key={index} className="text-gray-600">
                ...
                <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-blue-500"> (Read More)</a>
              </li>
            ))}
          </ul>
        </>
      ...
      {result.wikipedia_summary ? (
        <>
          ...
          <p className="text-gray-600">{result.wikipedia_summary.summary || "No summary available."}</p>
          <a href={result.wikipedia_summary.url || "#"} target="_blank" rel="noopener noreferrer" className="text-blue-500">Read More</a>
        </>
      ) : (
